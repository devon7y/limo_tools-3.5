...
chan_loc = '/Volumes/T7/ERP Files/Epoched Files/derivatives/limo_gp_level_chanlocs.mat';
files = '/Volumes/T7/ERP Files/Epoched Files/derivatives/LIMO_list_full_2.txt';
parameters = 1;
estimator = 'Weighted mean'; % 'Mean' 'Weighted mean' 'Trimmed mean' 'HD' 'Median'
analysis_type = 'Mean'; % 'Mean' 'Trimmed mean' 'HD' 'Median'
savename = ['/Volumes/T7/ERP Files/Epoched Files/derivatives/parameter_' num2str(parameters) '.mat'];
limo_central_tendency_and_ci(files, parameters, chan_loc, estimator, analysis_type, [],savename);

% Weighted means are taken from LIMO.design.weights. I am not sure how
% these weights are calculated. It could just be simple inverse variance.
...
% Get all line objects and use ColorOrder
h_lines = findobj(gca, 'Type', 'line');
colorOrder = get(gca, 'ColorOrder');

% Debug: check how many lines we have
fprintf('Found %d line objects\n', length(h_lines));

% Find the main ERP lines (typically the thickest/most visible ones)
line_widths = arrayfun(@(x) get(x, 'LineWidth'), h_lines);
[~, main_line_indices] = sort(line_widths, 'descend');
num_main_lines = length(average_files);

% Store the legend line handles
legend_handles = [];

% Only modify the main ERP lines (typically the thickest/most visible ones)
if length(h_lines) >= length(average_files)
    % Set colors for the main ERP lines
    for i = 1:num_main_lines
        if i <= size(colorOrder, 1)
            line_idx = main_line_indices(num_main_lines - i + 1);
            set(h_lines(line_idx), 'Color', colorOrder(i,:), 'LineWidth', 3);
            legend_handles(end+1) = h_lines(line_idx);
        end
    end
end

% Create legend with the correct line handles to ensure color matching
...
single_subject_files = ['/Volumes/T7/ERP Files/Epoched Files/derivatives/parameter_4_single_subjects_Weighted mean.mat'];
channel = [21];
...
plot_data = squeeze(paired_samples(:,:,4)); % t-values (4th dimension)
% p_values = squeeze(paired_samples(:,:,5)); % p-values (5th dimension)
p_values = squeeze(LIMO.cache.fig.pval); % p-values (5th dimension)
df_values = squeeze(paired_samples(:,:,3)); % df-values (3rd dimension)
...
cd '/Volumes/T7/ERP Files/Epoched Files/paired_ttest'
limo_display_results(1, ... # electodes x time graph
    'paired_samples_ttest_parameter_14.mat', ... # test statistics mat file
...
highlight_rects = {[-100, 21; 1500, 21], [-100, 36; 1500, 36], [-100, 101; 1500, 101], [-100, 224; 1500, 224]}; % Example: [200, 20; 400, 40], [600, 80; 800, 100]
...
time_channel_points_original = [550 36; 550 101; 700 153; 800 224];
...
imgax_exp = axes('Position', [0.1 0.08 0.75 0.55]); % Adjusted height to give title space
...
for n = 1:num_tf_points
    ...
    center_x = mean(xlims); center_y = mean(ylims);
...
h = colorbar(imgax_exp, 'Position', [0.87 0.08 0.03 0.55]); % Adjusted to match new main plot position
title(h, 't-values', 'FontSize', 10, 'Color', 'k');
...
set(imgax_exp, 'XTick', 0:250:1500);
