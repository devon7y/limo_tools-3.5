function [handle,Zi,grid,Xi,Yi] = limo_topoplot_dual(Values,loc_file,varargin)
...
CONTOURLINEWIDTH = 0.5; % default linewidth for contours
...
if nargs == 1
  if ischar(Values)
    if any(strcmp(lower(Values),{'example','demo'}))
      fprintf(['This is an example of an electrode location file,n',...
               'an ascii file consisting of the following four columns:n',...
               ' channel_number degrees arc_length channel_namenn',...
               'Example:n',...
               ' 1               -18    .352       Fp1 n',...
               ' 2                18    .352       Fp2 n',...
               ' 5               -90    .181       C3  n',...
               ...
               '13               180    .181       Pz  nn',...
                                                             ...
               'In topoplot() coordinates, 0 deg. points to the nose, positiven',...
               'angles point to the right hemisphere, and negative to the left.n',...
               'The model head sphere has a circumference of 2; the vertexn',...
               '(Cz) has arc_length 0. Locations with arc_length > 0.5 are belown',...
               'head center and are plotted outside the head cartoon.n',...
               'Option plotrad controls how much of this lower-head "skirt" is shown.n',...
               'Option headrad controls if and where the cartoon head will be drawn.n',...
               'Option intrad controls how many channels will be included in the interpolation.n',...
               ])
      return
    end
  end
...
if nargs < 2
  ...
  if ~exist(loc_file)
      fprintf('default locations file "%s" not found - specify chan_locs in topoplot() call.n',loc_file)
      error(' ')
  end
end
if isempty(loc_file)
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'colormap'
                ...
                % Store custom colormap instead of setting it globally
...
if exist('custom_colormap', 'var')
    cmap = custom_colormap;
elseif isempty(find(strcmp(varargin,'colormap')))
...
if strcmp(plotgrid,'on')
   ...
   %      fprintf('topoplot() warning: ''plotgrid'' channel matrix has duplicate channelsn');
   % end

   if ~isempty(plotchans)
     if intersect(gchans,abs(plotchans))
        fprintf('topoplot() warning: ''plotgrid'' and ''plotchans'' have channels in commonn');
     end
   end
end
...
if isempty(plotrad) && isfield(tmpeloc, 'plotrad'), 
    ...
    if plotrad < MINPLOTRAD || plotrad > 1.0
       fprintf('Bad value (%g) for plotrad.n',plotrad);
       error(' ');
    end
    if strcmpi(VERBOSE,'on') && ~isempty(plotrad)
       fprintf('Plotting radius plotrad (%g) set from EEG.chanlocs.n',plotrad);
    end
end
if isempty(plotrad) 
...
if isempty(headrad)  % never set -> defaults
  if plotrad >= rmax
     ...
     if strcmpi(VERBOSE, 'on')
       fprintf('topoplot(): not plotting cartoon head since plotrad (%5.4g) < 0.5n',...
                                                                    plotrad);
     end
  end
    ...
    if isempty(shrinkfactor) && isfield(tmpeloc, 'shrink'), 
        ...
        if strcmpi(VERBOSE,'on')
            if ischar(shrinkfactor)
                fprintf('Automatically shrinking coordinates to lie above the head perimter.n');
            else                
                fprintf('Automatically shrinking coordinates by %3.2fn', shrinkfactor);
            end
        end
    end
    ...
    if ischar(shrinkfactor)
        if strcmpi(shrinkfactor, 'on') || strcmpi(shrinkfactor, 'force') || strcmpi(shrinkfactor, 'auto')  
            if abs(headrad-rmax) > 1e-2
             fprintf('     NOTE -> the head cartoon will NOT accurately indicate the actual electrode locationsn');
            end
            if strcmpi(VERBOSE,'on')
                fprintf('     Shrink flag -> plotting cartoon head at plotradn');
            end
        ...
        if strcmpi(VERBOSE,'on')
            ...
            if abs(headrad-rmax) > 1e-2
                fprintf(' Warning: With this "shrink" setting, the cartoon head will NOT be anatomically correct.n');
            else
                fprintf('n');
            end
        end
...
if headrad ~= 0.5 && strcmpi(VERBOSE, 'on')
   ...
   fprintf(    ' ''headrad'' %-4.3gn',headrad);
   fprintf('Warning: The plotting radius of the cartoon head is NOT anatomically correct (0.5).n')
end
%
...
if length(pltchans) < length(Rd) && strcmpi(VERBOSE, 'on')
        fprintf('Interpolating %d and plotting %d of the %d scalp electrodes.n', ...
                   length(intchans),length(pltchans),length(Rd));    
end;


% fprintf('topoplot(): plotting %d channelsn',length(pltchans));
 ...
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  %
   if strcmpi(noplot, 'on') 
    if strcmpi(VERBOSE,'on')
       fprintf('topoplot(): no plot requested.n')
    end
    return;
   end
  %
  ...
  elseif strcmp(STYLE,'contour')                     % plot surface contours only
    [cls chs] = contour(Xi,Yi,ZiC,CONTOURNUM, 'LineWidth', CONTOURLINEWIDTH); 
    handle = chs;                                   % handle to a contourgroup object
    try, set(chs,'color',CCOLOR); catch, end % set color
  %
  %%%%%%%%%%%%%%%%%%%%%%%% Else plot map and contours %%%%%%%%%%%%%%%%%%%%%%%%%
  %
  elseif strcmp(STYLE,'both')  % plot interpolated surface and surface contours
      % Convert data to RGB colors using the custom colormap
      if exist('custom_colormap', 'var')
          % For smooth gradients, we need to interpolate to a much higher resolution
          % before converting to RGB
          high_res_scale = 512; % Much higher resolution for smooth gradients
          
          % Create high-resolution grid
          xi_hr = linspace(xmin,xmax,high_res_scale);
          yi_hr = linspace(ymin,ymax,high_res_scale);
          
          % Interpolate to high resolution
          try
              [Xi_hr,Yi_hr,Zi_hr] = griddata(inty,intx,double(intValues),yi_hr',xi_hr,'v4');
          catch
              [Xi_hr,Yi_hr] = meshgrid(yi_hr',xi_hr);
              Zi_hr = gdatav4(inty,intx,double(intValues), Xi_hr, Yi_hr);
          end
          
          % Mask outside the head
          mask_hr = (sqrt(Xi_hr.^2 + Yi_hr.^2) <= rmax);
          Zi_hr(~mask_hr) = NaN;
          
          % Normalize to [0,1] based on color limits
          Zi_norm = (Zi_hr - amin) / (amax - amin);
          Zi_norm(Zi_norm < 0) = 0;
          Zi_norm(Zi_norm > 1) = 1;
          
          % Convert to colormap indices with interpolation
          cmap_size = size(cmap,1);
          Zi_indices = Zi_norm * (cmap_size - 1) + 1;
          
          % Create high-resolution RGB image (vectorized for speed)
          [m,n] = size(Zi_hr);
          Zi_rgb = ones(m, n, 3); % Initialize with white
          
          % Find valid (non-NaN) points
          valid_mask = ~isnan(Zi_hr);
          
          if any(valid_mask(:))
              % Vectorized color mapping with interpolation
              idx = Zi_indices(valid_mask);
              idx_low = floor(idx);
              idx_high = ceil(idx);
              w = idx - idx_low;
              
              % Ensure indices are within bounds
              idx_low = max(1, min(idx_low, cmap_size));
              idx_high = max(1, min(idx_high, cmap_size));
              
              % Linear interpolation between colormap entries
              for c = 1:3
                  color_channel = zeros(m,n);
                  low_colors = cmap(idx_low, c);
                  high_colors = cmap(idx_high, c);
                  interpolated = (1-w) .* low_colors + w .* high_colors;
                  color_channel(valid_mask) = interpolated;
                  color_channel(~valid_mask) = 1; % White for NaN
                  Zi_rgb(:,:,c) = color_channel;
              end
          end
          
          % Plot as high-resolution RGB surface with interpolated shading
          if strcmp(SHADING,'interp')
              % For interp shading, use the high-res grid directly
              tmph = surface(Xi_hr*unsh,Yi_hr*unsh,zeros(size(Zi_hr))-0.1,...
                      'CData',Zi_rgb,'EdgeColor','none','FaceColor','interp');
          else
              % For flat shading, downsample back to original resolution
              % to maintain performance
              Zi_rgb_ds = Zi_rgb(1:high_res_scale/GRID_SCALE:end, 1:high_res_scale/GRID_SCALE:end, :);
              tmph = surface(Xi-delta/2,Yi-delta/2,zeros(size(Zi))-0.1,...
                      'CData',Zi_rgb_ds,'EdgeColor','none','FaceColor','flat');
          end
      else
          % Original code when no custom colormap
          if strcmp(SHADING,'interp')
  ...
  elseif strcmp(STYLE,'straight') || strcmp(STYLE,'map') % 'straight' was former arg
      % Convert data to RGB colors using the custom colormap
      if exist('custom_colormap', 'var')
          % For smooth gradients, interpolate to higher resolution
          high_res_scale = 512;
          ...
          % Plot as high-resolution RGB surface
          if strcmp(SHADING,'interp')
              tmph = surface(Xi_hr*unsh,Yi_hr*unsh,zeros(size(Zi_hr)),...
                      'CData',Zi_rgb,'EdgeColor','none','FaceColor','interp');
          else
              % For flat shading, downsample for performance
              Zi_rgb_ds = Zi_rgb(1:high_res_scale/GRID_SCALE:end, 1:high_res_scale/GRID_SCALE:end, :);
              tmph = surface(Xi-delta/2,Yi-delta/2,zeros(size(Zi)),...
                      'CData',Zi_rgb_ds,'EdgeColor','none','FaceColor','flat');
          end
      else
          % Original code when no custom colormap
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Set color axis  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %
% Only set coloraxis if not using custom colormap (RGB mode)
if ~exist('custom_colormap', 'var')
...
%
  ...
  set(gca,'Xlim',[-rmax rmax]*AXHEADFAC,'Ylim',[-rmax rmax]*AXHEADFAC)
   ...
   % fprintf('Current axes size %g,%gn',pos(3),pos(4));
...
%
elif strcmp(ELECTRODES,'labels')  % print electrode names (labels)
...
%
elif strcmp(ELECTRODES,'labelpoint') 
  if isempty(EMARKER2CHANS)
    hp2 = plot3(y,x,ones(size(x))*ELECTRODE_HEIGHT,...
        EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH);
  else
...
%
elif strcmp(ELECTRODES,'numpoint') 
  if isempty(EMARKER2CHANS)
    hp2 = plot3(y,x,ones(size(x))*ELECTRODE_HEIGHT,...
        EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH);
  else
...
%
elif strcmp(ELECTRODES,'numbers')
  for i = 1:size(labels,1)
    text(double(y(i)),double(x(i)),...
...
%
elif strcmp(ELECTRODES,'off') && ~isempty(EMARKER2CHANS)
    hp2b = plot3(y(mark2chans),x(mark2chans),ones(size((mark2chans)))*ELECTRODE_HEIGHT,...
        EMARKER2,'Color',EMARKER2COLOR,'markerfacecolor',EMARKER2COLOR,'linewidth',EMARKER2LINEWIDTH,'markersize',EMARKERSIZE2);
end
%
...
if ~isempty(DIPOLE)  
    ...
    if sum(DIPOLE(1,3:4).^2) <= 0.00001  
      if strcmpi(VERBOSE,'on')
        fprintf('Note: dipole is length 0 - not plottedn')
      end
      PLOT_DIPOLE = 0;
    end
    if 0 % sum(DIPOLE(1,1:2).^2) > plotrad
      if strcmpi(VERBOSE,'on')
        fprintf('Note: dipole is outside plotting area - not plottedn')
      end
      PLOT_DIPOLE = 0;
    end
    if PLOT_DIPOLE
