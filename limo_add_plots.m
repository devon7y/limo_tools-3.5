...
while out == 0
    ...
    if turn==1
        if subjects_plot == 1
            ...
            % Add these lines before the problematic plot command
            disp('Debugging dimensions:');
            disp(['Size of vect: ', num2str(size(vect))]);
            disp(['Size of Data(:,2): ', num2str(size(Data(:,2)))]);
            disp(['Size of Data(:,2)'': ', num2str(size(Data(:,2)'))]);
            disp(['Class of vect: ', class(vect)]);
            disp(['Class of Data: ', class(Data)]);
            
            % Check if variables contain NaN or Inf
            disp(['vect contains NaN: ', num2str(any(isnan(vect)))]);
            disp(['Data(:,2) contains NaN: ', num2str(any(isnan(Data(:,2))))]);

            % Debug what vect contains
            disp('=== VECT ANALYSIS ===');
            disp(['Length of vect: ', num2str(length(vect))]);
            disp(['First 10 values of vect: ', num2str(vect(1:min(10,length(vect))))]);
            disp(['Last 10 values of vect: ', num2str(vect(max(1,end-9):end))]);
            disp(['Min value: ', num2str(min(vect))]);
            disp(['Max value: ', num2str(max(vect))]);
            disp(['Step size (if uniform): ', num2str(vect(2)-vect(1))]);
            
            % Check if it's time, frequency, or sample indices
            if max(vect) > 1000
                disp('Likely represents: Time in milliseconds or frequency in Hz');
            elseif max(vect) <= 1 && min(vect) >= 0
                disp('Likely represents: Normalized time (0 to 1)');
            elseif min(vect) >= 1 && max(vect) <= 1000
                disp('Likely represents: Sample indices or frequency bins');
            else
                disp('Likely represents: Time in seconds or other units');
            end

            % Debug what Data(:,2) contains
            disp('=== DATA(:,2) ANALYSIS ===');
            disp(['Size of Data: ', mat2str(size(Data))]);
            disp(['Length of Data(:,2): ', num2str(length(Data(:,2)))]);
            disp(['First 10 values of Data(:,2): ', num2str(Data(1:min(10,size(Data,1)),2)')]);
            disp(['Last 10 values of Data(:,2): ', num2str(Data(max(1,end-9):end,2)')]);
            disp(['Min value in Data(:,2): ', num2str(min(Data(:,2)))]);
            disp(['Max value in Data(:,2): ', num2str(max(Data(:,2)))]);
            
            % Check what's in other columns of Data for context
            disp(['Number of columns in Data: ', num2str(size(Data,2))]);
            if size(Data,2) > 1
                disp(['First 5 values of Data(:,1): ', num2str(Data(1:min(5,size(Data,1)),1)')]);
                if size(Data,2) > 2
                    disp(['First 5 values of Data(:,3): ', num2str(Data(1:min(5,size(Data,1)),3)')]);
                end
            end
            
            % Check if there's a systematic pattern in what's missing
            disp(['Time vector length: ', num2str(length(vect))]);
            disp(['Data rows: ', num2str(size(Data,1))]);
            disp(['Difference: ', num2str(length(vect) - size(Data,1))]);

            % Check which time point is missing
            disp('=== IDENTIFYING MISSING TIME POINT ===');
            
            
            
            % Check if this is a systematic LIMO issue
            disp(['Sampling rate implied by time vector: ', num2str(1000/2), ' Hz']);
            disp(['Time vector spans: ', num2str(vect(end) - vect(1)), ' ms']);

            % Check if we can determine which boundary point is missing
            disp('=== CHECKING EPOCH BOUNDARIES ===');
            
            % Look for clues in the workspace about original epoch settings
            if exist('LIMO', 'var')
                if isfield(LIMO, 'data')
                    disp(['LIMO data start: ', num2str(LIMO.data.start)]);
                    disp(['LIMO data end: ', num2str(LIMO.data.end)]);
                    disp(['LIMO data size: ', mat2str(LIMO.data.size)]);
                    disp(['LIMO sampling rate: ', num2str(LIMO.data.sampling_rate)]);
                end
            end
            
            % Check if there are any other time-related variables in the workspace
            disp('Looking for time-related variables...');
            vars = who;
            time_vars = vars(contains(vars, {'time', 'Time', 'latenc', 'epoch'}, 'IgnoreCase', true));
            if ~isempty(time_vars)
                disp(['Found time-related variables: ', strjoin(time_vars, ', ')]);
            end
